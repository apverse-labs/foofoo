...
import {
  fetchLunchDinnerDishes, saveMealBuckets, fetchFOCuisineIds, fetchUserMealBuckets,
...
export default function OnboardingStep6() {
  ...
  useEffect(() => {
    PostHogService.screen('onboarding_step_6');
    (async () => {
      ...
      const dishes = await fetchLunchDinnerDishes(cuisineIds, profile?.food_pref ?? null);
  ...
  const handleComplete = async (buckets: BucketMap) => {
    ...
    try {
      ...
      Logger.info('STEP6', 'onboarding_step_complete', { step: 6, user_id: userId });
  ...
  return (
    <OnboardingLayout
      ...
      onNext={() => router.replace('/(onboarding)/step-7' as never)}
      onBack={() => router.replace('/(onboarding)/step-5' as never)}
      nextDisabled={!noData}
      hideFooter={!noData}
    >
      {loading ? (

 ...
 *
 * Secrets required — store in Supabase Dashboard → Integrations → Vault:
 *   CLOUDINARY_API_KEY    — Cloudinary API key
 *   CLOUDINARY_API_SECRET — Cloudinary API secret
 *
 * NOTE: Vault secrets are read via SQL (vault.decrypted_secrets view), NOT via
 * Deno.env.get(). SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are auto-injected
 * by the Supabase runtime and are always available via Deno.env.get().
 *
 * Invoke with:
...
Deno.serve(async (_req) => {
  try {
    ...
    const rows = (vaultRows ?? []) as Array<{ name: string; decrypted_secret: string }>;
    const apiKey    = rows.find(r => r.name === 'CLOUDINARY_API_KEY')?.decrypted_secret
                    ?? Deno.env.get('CLOUDINARY_API_KEY');   // fallback: Edge Fn secret
    const apiSecret = rows.find(r => r.name === 'CLOUDINARY_API_SECRET')?.decrypted_secret

...
| BUILD-09 | Admin / Data Operations | ✅ | DOC-27, DOC-28 | SCHEMA-RE-005: `re_taxonomy_releases` table (taxonomy_version, risk_level, qa_status, changed_entities, approved_by, rollback_plan). Applied to staging. `re-admin.repository.ts`: pure QA helpers `detectClassesWithNoDishes`, `detectAddonOnlyAsPrimary`, `isDietTagConsistent`; DB ops `runTaxonomyQAChecks`, `fetchActiveTaxonomyVersion`, `fetchTaxonomyReleases`, `createTaxonomyRelease`. 13 unit tests passing. |
| BUILD-10 | Analytics, QA, Experimentation & Governance | ✅ | DOC-25, DOC-26, DOC-28 | No new migration (reads from existing feedback/affinity tables). `re-analytics.repository.ts`: pure `computeAcceptanceMetrics`, `computeClassAcceptance`; DB ops `fetchUserSignalSummary`, `fetchUserAcceptanceMetrics`, `fetchClassAcceptanceMetrics`, `fetchConstraintViolationCount`. `re-governance.service.ts`: pure `parseSemver`, `bumpVersion`, `classifyVersionBump`, `riskLevelForBump`; `createChangelogEntry` (auto-derives risk from semver bump). 26 unit tests passing. 143 total RE unit tests passing across 9 suites. |
